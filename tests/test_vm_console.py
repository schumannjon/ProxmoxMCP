...
def mock_proxmox():
    ...
    # Two-phase exec: agent("exec").post() returns pid, agent("exec-status").get() returns output.
    # Mock.__call__ always returns .return_value regardless of args, so both calls share it.
...
async def test_execute_command_with_error_output(vm_console, mock_proxmox):
    ...
    assert result["success"] is True  # Success refers to API call, not command exit code
